...
func TestGetVersionedMigrationsToUndo(t *testing.T) {
	files := []MigrationFile{
		{Base: "00100200300001-init.undo.sql"},
		{Base: "00100200300002--users.undo.sql"},
	}

	historyRecords := []history.MigrateHistory{
		{MhName: "00100200300002--users.do.sql"},
		{MhName: "00100200300001-init.do.sql"},
	}

	result, err := getVersionedMigrationsToUndo(files, historyRecords, 2)
	assert.NoError(t, err)
	assert.Len(t, result, 2)
	assert.Equal(t, "00100200300002--users.undo.sql", result[0].Base)
	assert.Equal(t, "00100200300001-init.undo.sql", result[1].Base)
}
...
func TestFindCorrespondingUndoScript(t *testing.T) {
	...
	doScript := history.MigrateHistory{MhName: "00100200300002--users.do.sql"}

	result, found, err := findCorrespondingUndoScript(undoFiles, doScript)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.Equal(t, "00100200300002--users.undo.sql", result.Base)
}
